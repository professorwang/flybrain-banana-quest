...
With postsynaptic L1 normalization, both datasets show the same layered reachability under olfactory drive (200-tick probes, I=1.0), though the onset points differ per dataset: ti=1.0 dies at the mushroom-body entrance in both; ti=2.0 ignites the mushroom body in both, with FlyWire already producing its first descending spikes (GNG_DESC 10) while MaleCNS reaches none (DN 0); ti=3.0 makes descending neurons usable on FlyWire (3,810) but only just reaches them on MaleCNS (DN 11); ti=4.0 is the usable working point on MaleCNS (DN 75, still without visual-lobe runaway):
